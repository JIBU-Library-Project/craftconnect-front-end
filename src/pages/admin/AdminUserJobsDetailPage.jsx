...
function AdminUserJobsDetailPage() {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {job.declineReason && (
                <span className="text-xs text-yellow-700 bg-yellow-50 border-l-4 border-yellow-400 px-2 py-0.5 rounded">
                  Decline: {job.declineReason}
                </span>
              )}
              {job.cancellationReason && (
                <span className="text-xs text-red-700 bg-red-50 border-l-4 border-red-400 px-2 py-0.5 rounded">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4 sm:col-span-2">
            <h2 className="text-sm font-medium text-gray-700 mb-2">Artisan Information</h2>
            <dl className="space-y-1 text-sm text-gray-800">
              <div className="flex justify-between">
                <dt>Business</dt>
                <dd>{job.artisan?.businessName || "-"}</dd>
              </div>
              <div className="flex justify-between">
                <dt>Rating</dt>
                <dd>
                  {job.artisan?.rating ? `★ ${job.artisan.rating}` : "-"} (
                  {job.artisan?.completedJobs || 0} jobs)
                </dd>
              </div>
            </dl>
          </div>
...
export default AdminUserJobsDetailPage;
